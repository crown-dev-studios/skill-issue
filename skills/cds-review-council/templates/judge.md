...
Chat and `stdout` output are diagnostic only; they are not the authoritative output channel. The authoritative outputs for this stage are `summary.md`, `verdict.json`, and `done.json` in `{{ARTIFACT_DIR}}`.
...
- Finish by writing the required files completely, then end your turn or exit cleanly.
