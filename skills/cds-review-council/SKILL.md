...
- Skill directories available for `--skill-paths` (e.g., `~/.agents/skills/cds-architecture-review`)
...
npx @crown-dev-studios/skill-issue review-council \
  --target "<target>" \
  --skill-paths "/path/to/cds-architecture-review,/path/to/cds-plan-compliance" \
  --open-html
```
